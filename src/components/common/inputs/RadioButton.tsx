      ...
      <div className={`flex flex-col gap-1 ${containerClassName}`}>
        <label htmlFor={radioId} className="inline-flex items-center gap-2.5 cursor-pointer text-sm font-semibold text-slate-700 dark:text-slate-300 select-none">
          <input
            ...
            type="radio"
            id={radioId}
            className={`w-4.5 h-4.5 text-indigo-600 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:ring-indigo-500 focus:ring-offset-0 focus:outline-hidden transition-all duration-150 cursor-pointer ${className}`}
            {...props}
          />
